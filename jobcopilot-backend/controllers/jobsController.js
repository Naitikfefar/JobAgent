...
exports.searchJobs = async (req, res) => {
  try {
    ...
    const python = spawn(pythonCmd, [agentPath, JSON.stringify(searchParams)], {
    ...
    python.on('error', (error) => {
      ...
      return res.status(500).json({
        success: false,
        error: 'Job search agent is unavailable. Check Python and its dependencies on Render.',
      });
    });
    ...
    python.on('close', async (code) => {
      ...
      if (code !== 0) {
        ...
        return res.status(500).json({
          success: false,
          error: 'Job search agent failed. Check the Render service logs.',
          details: stderr.trim().slice(-1000),
        });
      }

      try {
        const rawJobs = JSON.parse(stdout.trim() || '[]');
