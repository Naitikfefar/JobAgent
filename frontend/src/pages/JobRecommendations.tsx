...
import { getTodayJobs, searchJobs, updateJobStatus } from '@/services/api';
import Sidebar from '@/components/layout/Sidebar';
import { useAuth } from '@/context/AuthContext';
import { Search, Filter, FileText, Mail, ExternalLink, CheckCircle2, XCircle } from 'lucide-react';
    ...
    <Sidebar>
      <div className="p-8 max-w-6xl mx-auto space-y-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-3xl font-bold text-slate-900">Job Recommendations</h1>
        ...
        {/* Filters */}
        <div className="card-soft p-4 flex flex-col sm:flex-row gap-4 items-stretch sm:items-center">
          <div className="flex-1 relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" />
            <input
              type="text"
              placeholder="Search jobs, companies..."
              className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:border-primary"
            />
          </div>
              ...
              <div className="p-6">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-start gap-4">
                    <div className="w-14 h-14 bg-slate-100 rounded-xl flex items-center justify-center text-2xl font-bold text-primary shrink-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap items-center gap-3 mt-2">
                        <span className="text-xs bg-slate-100 text-slate-700 px-2 py-1 rounded-full">{job.source}</span>
                        <span className="text-sm text-slate-600">{job.location}</span>
                        <span className="text-sm font-medium text-slate-900">{job.stipend}</span>
                      </div>
                    </div>
                  </div>
                ...
                <p className="text-slate-600 text-sm mb-4 line-clamp-2">{job.about}</p>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-2 px-4 py-2.5 rounded-lg border border-slate-200 hover:bg-slate-50 transition-all"
                  >
